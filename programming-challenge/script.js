const packageData = {
  dependencies: {
    "@mobiscroll/react": "^5.20.0",
    "@notifee/react-native": "7.4.0",
    "@react-native-clipboard/clipboard": "1.11.1",
    "@react-native-community/datetimepicker": "6.7.1",
    "@react-native-firebase/app": "17.3.1",
    "@react-native-firebase/dynamic-links": "^17.3.1",
    "@react-native-firebase/messaging": "17.3.1",
    "@react-native-picker/picker": "^2.4.8",
    "@react-navigation/bottom-tabs": "^6.5.2",
    "@react-navigation/native": "6.1.0",
    "@react-navigation/native-stack": "6.9.5",
    "@react-navigation/stack": "^6.3.10",
    "@sentry/react": "^7.20.1",
    "@sentry/tracing": "^7.20.1",
    "@stripe/stripe-react-native": "^0.27.2",
    "@tailwindcss/line-clamp": "^0.4.2",
    "@tanstack/react-query": "^4.2.1",
    "@types/rox-react-native": "^5.0.1",
    "async-retry": "^1.3.3",
    "auth0-js": "^9.19.0",
    axios: "^0.27.2",
    clsx: "^1.2.1",
    "core-js": "^3.6.5",
    "crypto-js": "^4.1.1",
    flowbite: "^1.5.2",
    "flowbite-react": "^0.1.10",
    formik: "^2.2.9",
    "framer-motion": "^7.3.5",
    i18next: "^21.9.1",
    "i18next-browser-languagedetector": "^6.1.5",
    "jwt-decode": "^3.1.2",
    lodash: "^4.17.21",
    "patch-package": "^6.5.1",
    react: "18.1.0",
    "react-currency-input-field": "^3.6.10",
    "react-datepicker": "^4.8.0",
    "react-dnd": "^16.0.1",
    "react-dnd-html5-backend": "^16.0.1",
    "react-dom": "18.1.0",
    "react-i18next": "^11.18.5",
    "react-icons": "^4.6.0",
    "react-native": "0.70.6",
    "react-native-auth0": "2.16.0",
    "react-native-biometrics": "^3.0.1",
    "react-native-calendars": "^1.1293.0",
    "react-native-crisp-chat-sdk": "0.12.0",
    "react-native-datepicker": "^1.7.2",
    "react-native-dropdown-picker": "^5.4.4",
    "react-native-gesture-handler": "^2.9.0",
    "react-native-get-random-values": "^1.8.0",
    "react-native-image-picker": "5.0.0",
    "react-native-localize": "^2.2.4",
    "react-native-render-html": "^6.3.4",
    "react-native-safe-area-context": "4.4.1",
    "react-native-screens": "3.18.2",
    "react-native-snap-carousel": "^4.0.0-beta.6",
    "react-native-toast-message": "^2.1.5",
    "react-native-url-polyfill": "^1.3.0",
    "react-native-webview": "^11.26.0",
    "react-navigation-stack": "^2.10.4",
    "react-qr-code": "^2.0.11",
    "react-responsive-carousel": "^3.2.23",
    "react-router-dom": "6.3.0",
    recoil: "0.7.6",
    "regenerator-runtime": "0.13.7",
    "rox-browser": "^5.4.2",
    "rox-react-native": "^5.4.2",
    slate: "^0.84.0",
    "slate-history": "^0.66.0",
    "slate-hyperscript": "^0.77.0",
    "slate-react": "^0.83.2",
    "svg-jest": "^1.0.1",
    tslib: "^2.3.0",
    "workbox-core": "^6.5.4",
    "workbox-expiration": "^6.5.4",
    "workbox-precaching": "^6.5.4",
    "workbox-routing": "^6.5.4",
    "workbox-strategies": "^6.5.4",
    yup: "^0.32.11",
  },
  devDependencies: {
    "@babel/preset-env": "^7.20.2",
    "@nrwl/cli": "15.2.1",
    "@nrwl/cypress": "15.2.1",
    "@nrwl/detox": "15.2.1",
    "@nrwl/eslint-plugin-nx": "15.2.1",
    "@nrwl/jest": "15.2.1",
    "@nrwl/linter": "15.2.1",
    "@nrwl/react": "15.2.1",
    "@nrwl/react-native": "15.2.1",
    "@nrwl/web": "15.2.1",
    "@nrwl/workspace": "15.2.1",
    "@react-native-async-storage/async-storage": "^1.17.10",
    "@react-native-community/cli": "9.1.1",
    "@react-native-community/cli-platform-android": "9.1.0",
    "@react-native-community/cli-platform-ios": "9.1.0",
    "@tailwindcss/typography": "^0.5.7",
    "@testing-library/jest-dom": "5.16.5",
    "@testing-library/jest-native": "5.3.0",
    "@testing-library/react": "13.3.0",
    "@testing-library/react-native": "11.2.0",
    "@types/auth0-js": "^9.14.6",
    "@types/crypto-js": "^4.1.1",
    "@types/jest": "28.1.1",
    "@types/node": "16.11.7",
    "@types/react": "18.0.20",
    "@types/react-datepicker": "^4.4.2",
    "@types/react-dom": "18.0.6",
    "@types/react-native": "0.70.4",
    "@types/react-router-dom": "5.3.3",
    "@types/rox-browser": "^5.0.2",
    "@typescript-eslint/eslint-plugin": "^5.29.0",
    "@typescript-eslint/parser": "^5.29.0",
    autoprefixer: "10.4.8",
    "babel-jest": "28.1.1",
    "babel-plugin-module-resolver": "4.1.0",
    cypress: "^10.2.0",
    detox: "19.12.5",
    eslint: "~8.15.0",
    "eslint-config-prettier": "8.1.0",
    "eslint-plugin-cypress": "^2.10.3",
    "eslint-plugin-import": "2.26.0",
    "eslint-plugin-jsx-a11y": "6.6.1",
    "eslint-plugin-react": "7.31.8",
    "eslint-plugin-react-hooks": "4.6.0",
    jest: "28.1.1",
    "jest-circus": "28.1.1",
    "jest-environment-jsdom": "28.1.1",
    "jest-react-native": "18.0.0",
    metro: "0.72.3",
    "metro-babel-register": "0.72.3",
    "metro-react-native-babel-preset": "0.72.3",
    "metro-react-native-babel-transformer": "0.72.3",
    "metro-resolver": "0.72.3",
    nx: "15.2.1",
    postcss: "8.4.16",
    prettier: "^2.6.2",
    "react-native-config": "1.4.6",
    "react-native-svg": "13.2.0",
    "react-native-svg-transformer": "1.0.0",
    "react-test-renderer": "18.1.0",
    tailwindcss: "3.1.8",
    "ts-jest": "28.0.5",
    "ts-node": "10.9.1",
    typescript: "~4.7.2",
    "webpack-merge": "^5.8.0",
    "workbox-webpack-plugin": "^6.5.4",
  },
  license: "MIT",
  name: "pilar",
  private: "true",
  scripts: {
    build: "nx build",
    start: "nx serve",
    test: "nx test",
    postinstall: "patch-package",
  },
  version: "0.0.0",
  volta: {
    node: "16.14.2",
  },
};

const packageDetails = {
  packageName: "",
  currentVersion: "",
  latestVersion: "",
  license: "",
};

const csvmaker = function (data) {
  csvRows = [];
  const values = Object.values(data).join(",");
  csvRows.push(values);
  return csvRows.join("\n");
};

const dependencies = packageData.dependencies;
const devDependencies = packageData.devDependencies;

async function getAvailablePackages(name) {
  const endpoint = `https://registry.npmjs.org/${name}`;
  const response = await fetch(endpoint);
  const data = await response.json();
  const packageName = data.name;
  const license = data.license;
  let currentVersion =
    packageData.dependencies[name] || packageData.devDependencies[name];

  const endpointForLatestVersion = `https://registry.npmjs.org/${name}/latest`;
  const res = await fetch(endpointForLatestVersion, {
    method: "GET",
  });

  const dataToFindVersion = await res.json();
  const latestVersion = dataToFindVersion.version;

  packageDetails.packageName = packageName;
  packageDetails.currentVersion = currentVersion;
  packageDetails.latestVersion = latestVersion;
  packageDetails.license = license;

  console.log(csvmaker(packageDetails));
}

Object.keys(dependencies).forEach((key) => {
  getAvailablePackages(key);
});

Object.keys(devDependencies).forEach((key) => {
  getAvailablePackages(key);
});
